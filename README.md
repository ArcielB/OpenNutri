...
- AI evidence highlighting is broad location guidance, not exact nutrient-coordinate matching.
  Queue and Approval build a compact deduplicated Sources strip from normalized payload rows. The scanner snaps matched evidence to whole detected table blocks or whole paragraph blocks, then the viewer scales those PDF-coordinate bounds onto the rendered page stage and draws every matched source overlay, clicked or not. Sources in the same table or paragraph share one deduplicated overlay; page-only hints scroll without coloring the whole page. Printed journal page hints such as `Page 95` are mapped to the actual PDF page when the viewer can detect header/footer page labels. Unmatched AI evidence remains visible as unverified.

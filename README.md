...
This is automated by GitHub Actions in `.github/workflows/daily-ops.yml` at 11:15 Europe/Istanbul (`08:15 UTC`), after the Gemini RPD reset at midnight Pacific time, and can also be run manually:
...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Daily recursive ops runner for automation. It first tops up reviewers from existing `human_review_ready` stock, then drains already queued AI work with `--stop-on-quota`, then crawls/uploads only when deficits remain, and stops for the day if Gemini quota is exhausted. If quota is hit after new human-ready papers are produced, it runs one final assignment pass before stopping.

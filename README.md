...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Quota-draining daily automation runner. It keeps Gemma screening fed with English crawler/upload batches, drains Gemma at configured stage RPM until daily quota/source/wall-clock stop, then drains Gemini at configured stage RPM from the highest-priority queued candidates. The scheduled workflow runs one long invocation instead of an outer 5-minute loop. JSON output reports `stopped_reason`, `terminal`, per-stage summaries, screened/routed/Gemini/human-ready totals, exhausted stages, and remaining queued candidates.

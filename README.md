...
- Shared general labeling queue: active labelers see the same available `human_review_ready` papers that already have a latest AI `has_data` normalized extraction payload, and a paper leaves the queue after the first general submission.
...
AI routing sends only normalized `has_data` outputs to human review when they are low-confidence or audit-sampled. Empty/no-usable-data AI results are finalized as AI empty outcomes rather than sent to labelers. `positive_threshold = 1.0` keeps all has-data extractions in human review for blind-study safety.
Daily ops are designed as a recursive AI-max loop:
`process already queued AI papers -> crawl/upload when no AI queue is available -> process the new AI queue -> repeat`.
The daily automation target is 20 Gemini calls, with at most 5 calls in each 5-minute controller iteration.
...
This is automated by GitHub Actions in `.github/workflows/daily-ops.yml` around the Gemini RPD reset. The workflow starts once at `00:05` America/Los_Angeles, then loops internally every 5 minutes until the orchestrator returns a terminal stop reason.
...
- `services/data-pipeline/scripts/process_stage_queue.py`: Claims one retry-fair batch of queued AI stage tasks per run, preferring lower `attempt_count`, then higher `priority`, then older creation order. It fetches reference foods/nutrients, runs `UnifiedEvaluator` with the nutrient catalog and text-matched food candidates in prompt, standardizes candidate rows into the human-submission-shaped `normalized_payload_json`, stores routed `ai_extractions`, sends only normalized has-data outputs to human review when low-confidence or audit-sampled, and finalizes empty/no-usable-data AI outcomes as empty. AI processing errors return the task and paper to `queued_for_ai` with `last_error` preserved for the next run, but repeated failures no longer monopolize every automated pass.
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Recursive ops runner for automation. It drains already queued AI work with `--stop-on-quota`, crawls/uploads when no AI queue is available, then processes the new AI queue. Each invocation returns a JSON `stopped_reason`, `terminal`, and cumulative daily AI count; the workflow controller repeats non-terminal invocations every 5 minutes until 20 Gemini calls are used or a terminal stop occurs. Default per-invocation AI budget is 5 tasks.

...
Daily ops are Gemma-target-led rather than fixed-iteration:
`crawl/refill until at least 1500 queued Gemma papers exist -> drain exactly the 1500 scheduled Gemma calls at stage RPM -> drain Gemini at stage RPM from highest-priority queued candidates until quota/source stop`.
...
Scheduled daily ops currently pace both Gemma and Gemini at `15` calls/minute and sleep `65` seconds after each full RPM window or per-minute quota event. The controller has no internal wall-clock cutoff in the scheduled workflow (`--max-wallclock-minutes 0`); the GitHub-hosted job still has the platform job cap. Gemma screening requeues stale `processing` tasks before queue decisions, preloads to `--screening-daily-target 1500`, requests English crawler/upload refills up to the remaining target, and stops as `source_exhausted` only after `3` consecutive prefill refills fail to increase queued Gemma work. Non-quota task errors past `AI_STAGE_MAX_TASK_ATTEMPTS=2` fail that task instead of retrying forever; quota/rate-limit errors still requeue without consuming meaningful attempts. Visible human queue stock does not stop daily AI draining.
...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Daily automation runner. It requeues stale stage tasks, preloads the Gemma screening queue to the configured daily target before spending Gemma calls, drains Gemma at configured stage RPM until the target is reached, then drains Gemini at configured stage RPM from the highest-priority queued candidates. The scheduled workflow runs one long unbuffered invocation instead of an outer 5-minute loop. JSON output reports `stopped_reason`, `terminal`, per-stage summaries, screened/routed/Gemini/human-ready totals, exhausted stages, and remaining queued candidates.

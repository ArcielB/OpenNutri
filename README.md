...
The pipeline is a 3-stage cascade: `gemma_proof_extraction_v1` (`gemma-4-31b-it`, retryable same-stage fallback `gemma-4-26b-a4b-it`) screens and ranks up to 1500 papers/day -> `gemini_flash_lite_triage_v1` (`gemini-3.1-flash-lite`, up to 500/day) re-ranks the strongest Gemma candidates -> `gemini_flash_db_payload_v2` (`gemini-3.5-flash`, safety ceiling 250/day) performs final extraction. These are quota ceilings rather than expected throughput; provider quota and available candidates normally stop a stage earlier. Each stage ranks via `score_followup_priority`, and priority-ordered claiming makes downstream stages process the strongest available candidates first.
...
`scheduled tick -> serialized refill controller requeues stale tasks, checks the recent Gemma failure circuit, counts active Gemma work, and tops up to at most 150 active Gemma tasks in bounded 30-paper crawler chunks without Supabase paper-PDF storage -> 5 drain-only workers (parallel to the controller) each claim up to 20 Gemma tasks, then interleave up to 10 Flash-Lite triage drains (<=500/Pacific-day) and up to 5 Gemini-final drains (<=250/Pacific-day safety ceiling) from ranked candidates`.
...
GitHub Actions requests `.github/workflows/daily-ops.yml` every 5 minutes, although GitHub may delay scheduled runs under load. Each invocation starts one serialized `refill-controller` job under the `daily-ops-refill-controller` concurrency group and a 5-job `drain-workers` matrix in parallel. Manual `workflow_dispatch` runs accept a `workers` input; inactive worker matrix entries skip checkout, setup, dependency install, and the drain step. The controller installs full crawler dependencies and keeps only a stable HuggingFace/model cache; workers install `services/data-pipeline/requirements-worker.txt` and do not restore run-id keyed data/model caches. The controller runs `daily_ops_orchestrator.py --controller-only --tick-mode`: it skips paper-storage cleanup/soft-limit checks, requeues stale Gemma/Flash-Lite/final-Gemini `processing` tasks, and stops crawler refill when at least 20 Gemma tasks failed with zero completions in the prior 6 hours. Otherwise it counts active Gemma work from `paper_stage_tasks.status IN ('queued', 'processing')` while excluding stale processing rows and crawls/registers English papers in chunks capped at 30 accepted PDFs. The crawler has a 2400-second wall-clock budget in scheduled controller runs and writes any accepted partial results before upload. The controller reports a soft queued Flash-Lite/final-Gemini reservoir target of 500 candidates but does not evict tasks. Workers run `daily_ops_orchestrator.py --drain-only --tick-mode --interleave-extraction`; they never crawl, upload, or refill, and they drain only already-created model tasks. R2-hosted PDFs are read through the authenticated S3-compatible endpoint, with the preserved publisher URL as a fallback, so workers do not depend on public `r2.dev` reachability. Task claiming remains DB-atomic, so overlapping scheduled worker matrices can safely process distinct rows. If Gemma source/refill is empty but queued downstream candidates and daily downstream quota remain, drain workers still process Flash-Lite and final Gemini slices instead of exiting early. Existing queued Gemma backlogs above the active target drain naturally without another 1500-paper crawl. The stage processor validates model runtime before claiming rows so missing API-key/config failures do not strand tasks in `processing`. Non-quota task errors past `AI_STAGE_MAX_TASK_ATTEMPTS=2` fail that task instead of retrying forever; task claims preserve the prior `last_error` until a new result is stored, and quota/rate-limit errors still requeue without consuming meaningful attempts. Visible human queue stock does not stop daily AI draining.
Cockpit views lazy-load their heavier data only when a cockpit tab is opened. The `Useful Papers`/dashboard cockpit payload uses the `get_cockpit_ai_extractions` RPC instead of `ai_extractions.select('*')`; the RPC returns the normalized payload and `raw_data.normalization_summary` but omits raw model responses and reasoning from the default list payload. Queue refreshes fetch only each paper's `latest_ai_extraction_id` prefill row instead of every extraction for that paper.
...
- `public.get_cockpit_ai_extractions(p_limit)`: Cockpit/approver RPC for the annotator's useful-paper AI summaries. It is intentionally slimmer than direct `ai_extractions.select('*')`: row identity, model/status/confidence/routing, `normalized_payload_json`, and a `raw_data` object containing only `normalization_summary`.
...
- `services/data-pipeline/scripts/r2_storage.py`: Cloudflare R2 (S3-compatible) helpers for hosting and reading paper PDFs - bucket `open-nutri`, free egress, immutable cache headers. Dormant unless the `R2_*` env vars/secrets are set; when active, `upload_to_supabase.py` re-hosts each validated PDF on R2 at registration (`papers.pdf_url` points at R2, the publisher URL is preserved in `papers.source_pdf_url`) and model workers read the object through the authenticated endpoint.
- `services/data-pipeline/scripts/rehost_pdfs_to_r2.py`: Resumable backfill that moves existing papers' PDFs to R2 (reviewer-facing statuses by default, `--all-statuses` for everything; rows already on R2 are skipped).

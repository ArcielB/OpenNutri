...
Daily ops are quota-led rather than fixed-iteration:
`crawl/refill enough Gemma work -> drain Gemma at stage RPM until its daily quota is exhausted or source work runs out -> drain Gemini at stage RPM from highest-priority queued candidates until quota/source stop`.
...
Scheduled daily ops currently pace both Gemma and Gemini at `15` calls/minute, sleep `65` seconds after each full RPM window or per-minute quota event, and run one long controller invocation up to `330` minutes inside the 6-hour GitHub Actions job. A quota/rate-limit after recent successful calls is treated as the per-minute limit; quota on the first call after cooldown marks that stage's daily quota exhausted and advances to the next stage. Gemma screening drains any existing queued work immediately, requeues stale `processing` tasks before queue decisions, and refills only when no Gemma work is queued. Scheduled refill is chunked at `5` English accepted papers per crawl/upload call while retaining the larger `75`-paper target as the replenishment setting, so AI starts after a small usable batch instead of waiting for a long full refill. Non-quota task errors past `AI_STAGE_MAX_TASK_ATTEMPTS=6` fail that task instead of retrying forever; quota/rate-limit errors still requeue without consuming meaningful attempts. Visible human queue stock does not stop daily AI draining.
...
Each orchestrator run stops with a machine-readable `stopped_reason`, `terminal`, per-stage summaries, `screened`, `routed_to_gemini`, `gemini_used`, `human_ready`, `quota_exhausted_stages`, and `remaining_queued`. AI processing still performs one final stock check if Gemini creates `human_review_ready` papers so the shared queue reflects new work immediately. This is automated by GitHub Actions in `.github/workflows/daily-ops.yml`, scheduled once per day at `07:17` UTC with no wall-clock gate for delayed runners.
`python3 services/data-pipeline/scripts/daily_ops_orchestrator.py --stage-rpm gemma_proof_extraction_v1=15,gemini_flash_db_payload_v2=15 --quota-cooldown-seconds 65 --max-wallclock-minutes 330 --screening-queue-low-watermark 30 --screening-refill-batch-en 75 --screening-refill-chunk-en 5`.
...
- `services/data-pipeline/scripts/upload_to_supabase.py`: Upload accepted PDFs to Supabase Storage, update `papers` by canonical identity, upsert metadata-stage discovery hits into `paper_search_hits` via deterministic `hit_key` values, and persist per-query batch history into `paper_search_batches` plus `paper_search_batch_hits`. Re-uploading a paper that already has a closed AI route or human outcome preserves its routing state instead of sending it through the currently active model again. Metadata-stage runs with zero accepted PDFs are now valid as long as search hits exist, and hits keep `paper_id` nullable until a paper row exists. Pass `--data-dir` or `--manifest`; the script now requires `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`.
...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Quota-draining daily automation runner. It requeues stale stage tasks, drains already queued Gemma work before crawling, keeps Gemma screening fed with chunked English crawler/upload batches, drains Gemma at configured stage RPM until daily quota/source/wall-clock stop, then drains Gemini at configured stage RPM from the highest-priority queued candidates. The scheduled workflow runs one long unbuffered invocation instead of an outer 5-minute loop. JSON output reports `stopped_reason`, `terminal`, per-stage summaries, screened/routed/Gemini/human-ready totals, exhausted stages, and remaining queued candidates.
...
- `AI_STAGE_MAX_TASK_ATTEMPTS` (optional retry ceiling for non-quota task errors; scheduled ops sets `6`)

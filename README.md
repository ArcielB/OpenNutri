...
- Shared general labeling queue: active labelers see the same available `human_review_ready` papers that already have a latest normalized AI decision payload (`has_data` or `no_usable_data`), and a paper leaves the queue after the first general submission.
...
AI routing sends both normalized `has_data` and `no_usable_data` outputs to human review when the matching confidence threshold is not met or the paper is audit-sampled. Thresholds of `1.0` are the blind-study safety setting: they keep AI decisions, including no-data decisions, in human review until accuracy is validated.
...
- `services/data-pipeline/scripts/process_stage_queue.py`: Claims one retry-fair batch of queued AI stage tasks per run, preferring lower `attempt_count`, then higher `priority`, then older creation order. It fetches reference foods/nutrients, runs `UnifiedEvaluator` with the nutrient catalog and text-matched food candidates in prompt, standardizes candidate rows into the human-submission-shaped `normalized_payload_json`, stores routed `ai_extractions`, sends low-confidence or audit-sampled AI decisions to human review, and finalizes only above-threshold non-audited AI outcomes. AI processing errors return the task and paper to `queued_for_ai` with `last_error` preserved for the next run, but repeated failures no longer monopolize every automated pass.

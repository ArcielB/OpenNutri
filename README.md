...
GitHub Actions runs `.github/workflows/daily-ops.yml` every 5 minutes. Each invocation runs one `daily_ops_orchestrator.py --tick-mode` pass and exits, so cancelled or delayed runners are recovered by the next scheduled tick. Gemma screening requeues stale `processing` tasks before queue decisions, uses `paper_stage_tasks.completed_at` since UTC midnight for daily counters, requests English crawler/upload refills at Gemma phase start or when the next slice lacks queued work, and then drains a bounded Gemma slice. Non-quota task errors past `AI_STAGE_MAX_TASK_ATTEMPTS=2` fail that task instead of retrying forever; quota/rate-limit errors still requeue without consuming meaningful attempts. Visible human queue stock does not stop daily AI draining.
...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Daily automation runner. In scheduled `--tick-mode`, it requeues stale stage tasks, counts completed Gemma/Gemini tasks since UTC midnight, preloads enough Gemma queue for the remaining 1500/day target at phase start, only tops up later when the next Gemma slice lacks queue, drains one bounded Gemma slice per tick while Gemma is below target, then drains one bounded Gemini slice per tick from the highest-priority queued candidates until the 20-call daily Gemini target is reached. JSON output reports `mode`, `day_start_utc`, `daily_completed`, `stopped_reason`, per-stage summaries, screened/routed/Gemini/human-ready totals, exhausted stages, and remaining queued candidates.

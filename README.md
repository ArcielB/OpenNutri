...
The runner stops cleanly when all reviewer queues are full, when Gemini quota/rate limit is hit, when the per-run AI task budget is exhausted, or when the configured cycle cap is reached.
If AI processing creates `human_review_ready` papers and then hits quota, the runner performs one final assignment pass before stopping so those papers can enter reviewer queues immediately.
This is automated by GitHub Actions in `.github/workflows/daily-ops.yml` around the Gemini RPD reset. The workflow wakes every 5 minutes across the UTC windows that can contain midnight Pacific, then a gate step runs real work only at `00:15, 00:20, ..., 00:55` America/Los_Angeles.
Each scheduled run uses `--max-ai-tasks 5` so it stays under the 5 RPM free-tier limit; when that per-run budget is exhausted, the next 5-minute scheduled run continues.
...
- `services/data-pipeline/scripts/daily_ops_orchestrator.py`: Daily recursive ops runner for automation. It first tops up reviewers from existing `human_review_ready` stock, then drains already queued AI work with `--stop-on-quota`, then crawls/uploads only when deficits remain, and stops for the day if Gemini quota is exhausted or the per-run AI task budget is used. If quota or budget is hit after new human-ready papers are produced, it runs one final assignment pass before stopping. Default target is 50 open assignments per reviewer and default AI budget is 5 tasks per run.

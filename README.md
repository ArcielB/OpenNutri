...
- AI evidence highlighting is broad location guidance, not exact nutrient-coordinate matching.
  Queue and Approval build a compact deduplicated Sources strip from normalized payload rows. The scanner returns PDF-coordinate bounds for matched table and paragraph evidence; the viewer scales those bounds onto the rendered page stage and draws every matched source overlay, clicked or not. Table labels draw visible green overlays around detected table regions, paragraph source quotes draw amber blocks around surrounding paragraphs, and page-only hints scroll without coloring the whole page. Printed journal page hints such as `Page 95` are mapped to the actual PDF page when the viewer can detect header/footer page labels. Unmatched AI evidence remains visible as unverified.

...
data are preserved; idle storage is not being deleted.

The dormant pipeline's language scope is English only. The Turkish/DergiPark crawler path remains in the codebase for later reactivation, but refill and daily ops defaults request `tr=0` and search Europe PMC, OpenAlex, and Semantic Scholar only.
...
When deliberately run, daily ops use resumable ticks rather than one long controller:
`manual tick -> serialized refill controller requeues stale tasks, checks the recent Gemma failure circuit, counts active Gemma work, and tops up to at most 150 active Gemma tasks in bounded 30-paper crawler chunks without Supabase paper-PDF storage -> 5 drain-only workers (parallel to the controller) each claim up to 20 Gemma tasks, then interleave up to 10 Flash-Lite triage drains (<=500/Pacific-day) and up to 5 Gemini-final drains (<=250/Pacific-day safety ceiling) from ranked candidates`.
...
The dormant `.github/workflows/daily-ops.yml` retains `workflow_dispatch` for a
reviewed restart, but its GitHub workflow state must also be explicitly re-enabled.
When dispatched, it starts one serialized `refill-controller` and a configurable
five-job `drain-workers` matrix. The controller is the only crawler/uploader; workers
only drain already-created tasks. R2 reads, retry behavior, failure circuits, and
quota controls remain implemented and tested, but none run while the workflow is
disabled.
...
When manually enabled, the workflow runs on GitHub-hosted infrastructure rather than
this laptop. It talks to Supabase Postgres/REST, Cloudflare R2, and Gemini using
...
- `GEMINI_API_KEY`
- `OPENNUTRI_CORE_DB_PATH` (optional; bundled Core `v0.3.0` by default)
- `OPENNUTRI_SEMANTIC_INDEX_VERSION` (rebuild the complete index when its model or
  dimensions change)

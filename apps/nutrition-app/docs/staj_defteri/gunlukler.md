...
Başlangıçta main.dart içinde Flutter bağlamının hazırlanması, yerel kayıtların yüklenmesi ve uygulamanın açılması sırasını inceledim.

Supabase hazırlığı beklenmeden başlatılıyor; günlük arayüzü uzak kimlik doğrulama servisini beklemiyor. Yerel denetleyicinin hazırlanması ise runApp öncesinde tamamlanıyor. Böylece daha önce kaydedilmiş öğünlerin gösterilmesiyle yapay zekâ bağlantısının hazır olması birbirinden ayrılmış oluyor.

Testlerde gerçek depolama veya ağ istemcisi yerine kontrollü nesneler verilebilmesini de kullandım. Örneğin LocalStore yerine kaydetmeyi bekleten bir test nesnesi, normal kullanımda yakalanması zor yazma sırası sorunlarını görünür kılıyor. Bu uygulama sayesinde bileşenlerin girdisini, çıktısını ve sorumluluğunu belirleme konusunda deneyim kazandım.
...
Ana ekranın kayıt davranışını home_shell_test.dart içindeki senaryoyla değerlendirdim. Depolama tamamlanmadan yeni besinin görünmesi, arayüzün gereksiz beklemediğini gösteriyor; kalıcı kaydın sonucu ise denetleyicide ayrıca takip ediliyor. Bu çalışma sayesinde görsel tasarımla veri akışını birlikte düşünmeyi öğrendim. Özellikle tarih başlığı, öğün listesi ve toplamların birlikte güncellenmesi üzerinde durdum.
...
Arama sonuçlarından seçim yapıldığında önce besinin ayrıntısını yükleyip ardından porsiyon penceresini açan akışı bağladım. Kullanıcı porsiyonu onaylarsa bu pencere DiaryEntry döndürüyor; iptal ederse kayıt oluşmuyor. Ayrıntıları besin kimliğine göre bellekte saklayan önbellek, aynı besin tekrar açıldığında gereksiz ağ çağrılarını azaltıyor. Önbelleği, açık oturumdaki ayrıntı isteklerini hızlandıran bir katman olarak kullandım.

İsteklere 30 saniyelik zaman aşımı uygulanmasını ve hatanın arayüzde yakalanmasını inceledim. Beklenen akışı “sorgu → kısa sonuç → ayrıntı → porsiyon → günlük kaydı” şeklinde takip ederek veri dönüşümlerini netleştirdim. Bu çalışma, API entegrasyonunun yalnızca adres çağırmak olmadığını gösterdi; boş girdi, başarısız yanıt ve kullanıcının işlemi iptal etmesi de akışın parçalarıdır. Hazır besin servisinin mobil uygulamaya entegrasyonunu bu katmanda düzenledim.
...
Öncelikle her tuş vuruşunda istek göndermemek için 350 milisaniyelik gecikmeli aramayı kullandım. Yeni bir değişiklik olduğunda önceki zamanlayıcı iptal ediliyor ve yeni süre başlıyor.

Ancak bu yöntem, daha önce gönderilmiş bir isteği tek başına geçersiz kılmıyor. Bu nedenle her değişiklikte istek numarasını artırıp, dönen yanıtın numarasını güncel numarayla karşılaştırdım. Yalnızca son aramaya ait yanıt ekran durumunu güncelliyor.
...
Regresyon testinde isteğin yanıtını Completer ile bekletip arama alanını temizleyen senaryoyu kullandım. Sonradan tamamlanan yanıtın ekranda besin göstermemesi bekleniyor. Böylece yalnızca hızlı internet üzerinde düzgün görünen bir ekranı değil, yanıt sırası değiştiğinde de doğru davranan akışı değerlendirdim. Bu test sayesinde kullanıcının son isteğini temsil eden yanıtı seçme ve eski sonuçları ayıklama konusunda pratik kazandım.
...
Miktar düzenlemede sonlu, sıfırdan büyük ve en fazla 10.000 gram değer kabul edilmesini inceledim. Virgüllü ondalık girişin noktaya çevrilmesi, Türkçe kullanım için önemlidir. Satın alınan ağırlık seçilmiş ancak uygun katsayı bulunamamışsa kayıt düzeltme gerektiriyor. Bu uygulama, miktar doğrulamasıyla ağırlık dönüşümünü aynı hesaplama akışında birleştirmemi sağladı.
...
Kaydetme sırasında her DiaryEntry nesnesi toJson ile alanlarına ayrılıyor; oluşan liste jsonEncode ile metne dönüştürülüyor. Açılışta getString ile okunan içerik jsonDecode ve fromJson üzerinden tekrar nesnelere çevriliyor. Henüz kayıt yoksa boş liste dönülüyor. Yalnızca foodId saklamak yerine miktar, tarih ve hesaplanmış besin değerlerini birlikte tutmak, geçmiş öğünlerin kaynak servisine yeniden bağlanmadan gösterilmesini sağlıyor. saveEntries içindeki setString sonucunu kontrol ettim. Depolama işlemi false dönerse StateError üretiliyor; böylece çağıran ekran başarısız yazmayı tamamlanmış gibi kabul etmiyor. Testlerde gerçek kişisel günlük yerine bellek içi saklama nesneleri ve SharedPreferences test başlangıç değerleri kullanılıyor. JSON dönüşüm senaryosu kimlik, öğün, gram ve kalori bilgisinin geri okunduğunda korunmasını sınayarak veri kaybına karşı somut kontrol sağlıyor.

Son olarak yerel saklama ile açılış akışının bağlantısını takip ettim. Denetleyici kayıtları yükledikten sonra seçilen tarihe ait öğünleri aynı modeller üzerinden görüntülüyor. Verilerin ayrı anahtarlarda tutulması ve kayıt anındaki besin değerlerinin korunması bu akışı anlaşılır kılıyor. Çalışma boyunca JSON dönüşümü, yazma sonucunun kontrolü ve kayıtların yeniden okunması konularında pratik deneyim kazandım.
...
Regresyon testinde ilk yazmayı bekletip düzeltmeyi sıraya aldım; ikinci yazmaya “disk full” hatası verilince kaydın son saklanan 100 grama dönmesi bekleniyor. Bu kontrollü hata, normal bir denemede kolayca görünmeyen yarış durumunu anlamamı sağladı. Bu çalışma, işlem sırasını ve kimlik denetimini hızlı arayüz davranışıyla birlikte tasarlama becerimi geliştirdi.

## 12 | Kayıt Düzenleme ve İptalde Verinin Korunması

Bugün kaydedilmiş bir öğünü düzenlerken özgün verinin kaybolmaması üzerine çalıştım. Özellikle sesli kaydın ardından Edit batch açıldığında mevcut besinleri önce silmek risklidir. Kullanıcı yalnızca kontrol etmek için pencereyi açıp geri dönebilir. Editör açıldığında ve iptal edildiğinde özgün kayıtları korudum. Save changes başlatılınca bellek ve arayüz hemen güncelleniyor, disk yazması sıraya alınıyor. Başarı bildirimi yazma tamamlandıktan sonra veriliyor. Yazma başarısızsa ve bu değişiklik hâlâ güncelse son saklanan duruma dönülüyor.
...
Enerji hesabında karşılaşılan önemli ayrıntı, USDA kaynaklarının aynı bilgiyi farklı alanlarda taşıyabilmesiydi. Bir besinde Energy, diğerinde Energy (Atwater Specific Factors) bulunabiliyor. Önce bütün alanları topladıktan sonra yalnızca Energy alanını seçmek, diğer besinin kalorisini kaybettiriyordu.

Çözümde her DiaryEntry için öncelikle geçerli enerji alanını seçtim; günlük toplamı daha sonra bu tekil kalori değerlerinden oluşturdum.
...
Protein, karbonhidrat ve yağ toplamlarını da seçilen güne ait kayıtlar üzerinden hesapladım. Model testi, 100 ve 50 gramlık kontrollü elma kayıtlarının toplam enerjisinin 78 kcal olduğunu doğruluyor. Kullanıcı başka güne geçtiğinde aynı kayıtların yanlışlıkla toplamda kalmaması gerekiyor. Beklenen sayıyı elle hesaplayıp test sonucuyla karşılaştırmak, kaynak alanlarını doğru seçtiğimi değerlendirmemi sağladı.
...
Testte günlüğe eklenmiş üç besin kaydı kullanılıyor: birinde 5 mikrogram D vitamini, ikincisinde sıfır, üçüncüsünde ise hiç alan bulunmuyor. Beklenen toplam 5, bilinen kayıt sayısı 2 ve toplam kayıt sayısı 3. Aynı örnekte kalsiyum bilgisi hiç yoksa null kalıyor. Mikrogram için µg, ug ve mcg gibi farklı yazımların aynı birime dönüştürülmesi de kontrol ediliyor. Sıfırın veri olarak korunması, eksik bilgiyle karıştırılmaması açısından önemliydi.

Raporun açıklamalarında günlük değerleri, birimleri ve genel referansları anlaşılır biçimde gösterdim. Kişinin elle belirlediği hedefleri ayrıca ele alarak raporun beslenme takibi amacıyla okunmasını sağladım. Bu çalışma bana raporlama kalitesinin yalnızca çok sayıda gösterge eklemek olmadığını öğretti. Bilginin kaynağını, birimini ve eksik kalan kısmını açıklamak, sonucun kullanıcı tarafından doğru anlaşılması için gereklidir.
...
Regresyon testi, diyet değişince formun güncellenmesini ve ilgisiz bildirimde kaydedilmemiş metnin korunmasını birlikte denetliyor. Ayrıca kişisel hedef değişiminin günlük öneriyi yeniden yüklenen denetleyicide de geçersiz bırakması sınanıyor. Bu senaryolarla formdaki geçici girdiyi koruma ve dışarıdan gelen hedef değişikliğine uyum sağlama konularında deneyim kazandım.
...
Kayıt durumlarını boşta, kaydediyor, durdu, izin reddedildi ve hata şeklinde ayırdım. Kayıt başlatılamadığında geçici dosya temizleniyor ve ekran uygun duruma dönüyor. Dosya tamamlandıktan sonra çözümleme isteğine geçiliyor. Böylece izin kontrolü, kayıt durumu, dosya yaşam döngüsü ve servis bağlantısını adım adım takip edebildiğim bir yapı oluştu.
...
Gerçek kayıt sınıfında eşik -40 dBFS, konuşma sonrasındaki sessizlik süresi 1,6 saniye olarak ayarlanmış. Eşiğin üstünde ses algılandığında sessizliğin başlangıcı sıfırlanıyor; düşük seviye yeterince uzun sürerse stop çağrılıyor.

Buna ek olarak 30 saniyelik kesin süre sınırı bulunuyor. Böylece gürültülü ortamda sessizlik algılanmasa bile tek kaydın süresi sınırsız büyümüyor. Ses seviyesi ve geçen süreyi birlikte değerlendirerek kayıt sonunu belirleyen bu mantığı uygulamalı olarak inceledim.

Durdurma işleminin hem düğmeden hem zamanlayıcıdan tetiklenebilmesini dikkate aldım. _stopping işareti aynı dosyanın iki kez durdurulmasını sınırlıyor. Kayıt bittiğinde ses seviyesi aboneliği ve zamanlayıcı kapatılıyor; iptal veya işleme sonrasında geçici dosya siliniyor. Kaynakları yalnızca başarılı yolda temizlemek yeterli olmadığından hata ve iptal yollarını da takip ettim. Zamanlayıcı ve aboneliklerin aynı durdurma akışından kapatılması, kaynak yönetimini takip etmemi kolaylaştırdı.

Birim testinde gerçek mikrofon yerine belirli zamanlarda verilen -60 ve -20 dBFS değerleri kullanılıyor. Test, başlangıç fırsatını ve yeniden yükselen sesin sessizlik sayacını sıfırlamasını kontrol ediyor. Test nesnesinin varsayılan son sessizliği 2 saniyedir; gerçek kaydedici bunu 1,6 saniyeye ayarlar. Bu ayrımı görmek, testteki sayıyı doğrudan çalışma ayarı sanmamayı öğretti. Süre, eşik ve kaynak temizliğini birlikte ele alarak zamanlayıcılı işlemler konusundaki bilgimi pekiştirdim.

## 18 | Ses Servisine Güvenli İstek Gönderilmesi

Bugün tamamlanmış ses dosyasını mevcut çözümleme servisine gönderen mobil istemciyi çalıştım. VoiceApiClient, doğrudan Gemini'ye gizli anahtarla bağlanmak yerine uygulamaya ayrılmış kimlik doğrulama oturumunu kullanıyor. Oturum yoksa anonim giriş yapılıyor ve erişim belirteci Authorization: Bearer başlığına ekleniyor. Buradaki anonimliğin ses içeriğinde kişisel bilgi bulunamayacağı anlamına gelmediğini, yalnızca kullanıcıya ayrı bir hesap formu açılmadığını ayırdım. POST /v1/voice/resolve isteğini multipart biçiminde hazırladım. Dosya audio alanına meal.wav adı ve audio/wav içerik türüyle ekleniyor. language_hint, local_timestamp ve timezone alanları konuşmanın dili ve zaman bağlamını taşıyor. İngilizce ve Türkçe cihaz yerelleri desteklenen ipuçlarına dönüştürülüyor; diğerlerinde otomatik dil seçimi isteniyor. Dosya 1 MB sınırını aşıyorsa gönderimden önce hata veriliyor.
...
Besin ayrıntısı yüklenemezse konuşma sonucunun tamamen başarısız sayılmamasına dikkat ettim. Ekran, öğünün anlaşıldığını fakat sayısal ayrıntıların yüklenemediğini açıklayabiliyor; yeniden deneme veya elle arama sunuyor. Geçersiz yapılandırılmış yanıt senaryosu da korunabilen konuşma metninin ekranda kalmasını denetliyor. Bu süreçte konuşma metni, aday besin ve kaynak ayrıntısını ayrı durumlar olarak yönetmeyi öğrendim; yeniden deneme akışını bu ayrım üzerine kurdum.
...
Miktar söylenmemişse 100 gram, ağırlık temeli belirtilmemişse yenilebilir ağırlık varsayılanını kullandım. Bu değerleri ölçülmüş bilgi gibi sunmadım; belirsiz eşleşmeler ve varsayılanlar needsReview üzerinden Quick estimate olarak işaretleniyor. Kullanıcı açıkça satın alınan ağırlık belirtmiş fakat seçilen besinde kullanılabilir dönüşüm katsayısı yoksa kayıt engelleniyor. Bütün öğeler kullanılabilir olduğunda grup birlikte kaydediliyor; düzeltme gereken gruplar ortak inceleme ekranına yönlendiriliyor.
...
Sesli kayıt testlerinde miktarsız yanıtın tahmin olarak eklenmesi, verilen gram miktarının korunması ve geri bildirimin başarı ekranını geciktirmemesi denetleniyor. Düzenleme iptal edildiğinde özgün kayıtların korunması da aynı akışın devamıdır. Bu çalışma sayesinde otomatik ekleme, görünür tahmin işareti ve sonradan düzeltme seçeneklerini tek bir kullanıcı akışında birleştirdim.
...
Testte yerel saklamayı yapay olarak bekleterek kapanış çağrısının erken gelmediğini kontrol ettim. Saklama tamamlanınca kapanışa izin verilmesi, “söyle ve dön” akışının veri kaybetmeden tamamlanmasını sağlıyor. Görünür kayıt ekranı, saklama tamamlanana kadar işlemin durumunu gösteriyor. Bu uygulama sayesinde araç takımının kurulumunu, tarih seçimini ve kayıt sonrası kapanışını tek bir akışta ilişkilendirdim.
...
targetsForCalories yöntemi kullanıcının mevcut kalori hedefini koruyup makro hedeflerini hesaplıyor. Kalori hedefi her makronun enerji payıyla çarpılıyor; bulunan enerji protein ve karbonhidrat için 4'e, yağ için 9'a bölünerek gram değerine çevriliyor. Kodun dengeli şablonunu açıklamak için 2.000 kcal örneğini kullandım: yüzde 25 protein 125 gram, yüzde 45 karbonhidrat 225 gram, yüzde 30 yağ yaklaşık 66,7 gram verir. Bu örnek, mevcut kalori hedefinden makro hedeflerine geçişi açıklıyor.
...
personalization_test.dart içindeki senaryo şablonun hedefe uyarlanmasını ve sonradan düzenlenebilmesini kontrol ediyor. Şablon değişikliği, eski günlük önerinin temizlenmesini de tetikliyor. Şablon seçimini makro hedefleri ve öneri bağlamıyla ilişkilendirerek kişiselleştirme akışını tamamladım. Bu çalışma, kullanıcının seçiminin hesaplamalara ve diğer ekranlara tutarlı yansımasını sağlamamı kolaylaştırdı.
...
Önbellek testinde önce öneriyi saklayıp diyet, hedef veya tercih değişikliği uyguladım; ardından yeni denetleyiciyle veriyi tekrar yükleyen senaryoyu değerlendirdim. Eski önerinin null gelmesi, temizliğin yalnızca ekranda yapılmadığını doğruluyor. Tarih testi de kartın yanlış gün altında görünmemesini denetliyor. Bu sayede önbelleğin geçerliliğini tarih ve kullanıcı tercihleriyle birlikte yönetme konusunda pratik kazandım.
...
Görüşme ile kalıcı tercih belleğini ayrı tuttum. Ekrandaki konuşma geçmişi oturumluktur ve isteğe en fazla son altı mesaj bağlam olarak aktarılıyor. CoachMemory ise açık kullanıcı ifadelerinden çıkarılan kısa bilgileri saklıyor. Örneğin “Et yemiyorum” ifadesi bir tercih adayı olabilir; yalnızca belirli bir besin hakkında soru sormak, kullanıcının onu tükettiğini veya ona alerjisi olduğunu kanıtlamaz. Tercih adaylarını kullanıcının açık ifadeleriyle ilişkilendirmeye dikkat ettim.

Servisten dönen memoryUpdates alanını profil yönetimine bağladım. Aynı bilginin tekrarlanmasını sınırlayan denetim bulunuyor ve en fazla 30 tercih saklanıyor. Eklenen bilgiler yanıtta gösteriliyor; ayarlardan tek tek silinebiliyor. Silme, sonraki önerilerde gönderilen bağlamı da değiştiriyor. Model talimatında tercih adaylarının açık kullanıcı ifadelerinden çıkarılması isteniyor; uygulama da bu adayları görünür ve silinebilir biçimde sunuyor.

Sesli görüşmede aynı sınırlı kayıt bileşeninden yararlanılıyor, fakat amaç besini günlüğe eklemek değil koça mesaj göndermektir. Geçici dosya işlem sonunda temizleniyor; yanıtta varsa konuşma metni ve cevaplayan model gösteriliyor. Profil testleri saklama ve silme davranışlarını değerlendirmeye yardımcı oluyor. Bu çalışma, görüşme bağlamı ile kalıcı tercihi ayırma ve kullanıcıya saklanan bilgi üzerinde kontrol sağlama becerimi geliştirdi.
...
Testlerde Oracle'ın görünmeden istek başlatmaması ve kota hatasını bağlantı kesintisinden ayırması ele alınıyor. Oracle'ın bağlama dayalı besin fikirlerini arama ve porsiyon seçimine bağlayarak öneriden kayda uzanan akışı oluşturdum. Bu sayede model yanıtlarını uygulamanın doğrulanmış besin verisiyle birlikte kullanma konusunda deneyim kazandım.
...
Son olarak doğrulama çıktısını değerlendirdim: flutter analyze --no-pub temiz sonuç verdi ve flutter test --no-pub --reporter expanded ile 44 Flutter testi geçti. Sonuçları hesaplama, veri saklama ve ekran etkileşimi senaryolarıyla ilişkilendirdim. Testlerin beklenen davranışı açık biçimde tanımlaması, düzeltmelerin etkisini göstermemi ve uygulama kararlarını teknik olarak açıklamamı kolaylaştırdı.

## 30 | Android Paketinin Hazırlanması ve Genel Değerlendirme

Son çalışma gününde Android paketleme ve teslim sürecini toparladım. pubspec.yaml içindeki 1.1.2+5 bilgisinde kullanıcıya gösterilen sürümle derleme numarasını ayırdım. flutter build apk --release komutu kurulum paketini üretir; uygulamaya ait herkese açık kimlik doğrulama yapılandırması derleme sırasında verilir. Gizli sunucu anahtarlarının pakete eklenmemesi gerekiyor. Derleme ayarları, sürüm bilgisi ve kurulum çıktısı arasındaki ilişkiyi takip ettim.

Paket çıktısının build/app/outputs/flutter-apk/app-release.apk altında oluştuğunu ve güncellemede adb install -r komutunun mevcut uygulamanın üzerine kurulum için kullanıldığını inceledim. Sürümün kurulum ve cihaz kontrol kayıtlarını paketleme adımlarıyla birlikte değerlendirdim. Android'in uygulama kimliği ve imza üzerinden güncellemeyi tanıması, kurulum sürekliliğini anlamamı sağladı.

Teslim akışını uygulamanın günlük kullanım sırasına göre toparladım: besin arama, porsiyon seçme, kaydı düzenleme, günlük toplamı görme, sesli kayıt ve araç takımıyla hızlı giriş. Ardından hedef ve diyet seçimiyle günlük öneri, koç görüşmesi ve Oracle bağlantısını ilişkilendirdim.

README kullanım ve doğrulama komutlarını, davranış belgesi mimariyi, denetim kaydı test sonuçlarını, BACKLOG ise tamamlanmamış işleri açıklıyor. Böylece sonraki kişi yalnızca kurulum dosyasına bağımlı kalmıyor.

Genel değerlendirmede besin günlüğü, miktar hesaplamaları, sesli kayıt, Android araç takımı ve kişiselleştirilmiş önerilerin ortak veri yapısı üzerinde bir araya geldiğini gördüm. Bu çalışma boyunca arayüz, API, cihaz özellikleri, hesaplama ve testlerin birbirine bağlı olduğunu öğrendim. En önemli kazanımım, bir özelliği yalnızca çalışır göstermekle yetinmeyip hangi veriyle, hangi koşulda ve hata olduğunda nasıl davrandığını açıklayabilecek bir geliştirme yaklaşımı edinmek oldu.

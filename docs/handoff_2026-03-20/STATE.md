...
- Daily ops uses `gemma_proof_extraction_v1` with `gemma-4-31b-it` before Gemini, with `gemma-4-26b-a4b-it` configured as a same-stage fallback for retryable 31B failures. The final extraction stage key remains `gemini_flash_db_payload_v2` and uses stable `gemini-3.5-flash`. Gemma-positive papers enqueue Gemini by priority, and scheduled ops now run as recurring ticks keyed to stage-specific quota-day completion counts: Gemma defaults to UTC reset, Gemini defaults to `America/Los_Angeles` reset. The serialized controller tops up only enough queued/non-stale-processing Gemma work to keep about 150 active tasks, drain workers process bounded Gemma slices of 20 tasks each, and workers interleave a small Gemini slice from already-ranked candidates so useful papers can reach humans before the full Gemma target completes. If Gemma source/refill is empty while queued Gemini candidates and daily Gemini quota remain, workers still drain Gemini instead of exiting early. Existing Gemma backlogs above the active target should drain naturally without a new 1500-paper crawl.
...
3. Controller counts completed Gemma `paper_stage_tasks` since the configured Gemma quota-day start (default UTC) and completed Gemini tasks since the configured Gemini quota-day start (default `America/Los_Angeles`).
4. Controller counts active Gemma work from `paper_stage_tasks.status IN ('queued', 'processing')`, excluding stale processing rows. If today's completed Gemma count is below 1500 and active Gemma work is below `--screening-active-target 150`, it crawls/uploads only `min(remaining_today, 150) - active_count` English papers using bounded 150-paper batch/chunk settings and `--refill-step-tr 0`. It also reports the soft queued-Gemini reservoir target of 500 candidates but does not evict existing tasks.

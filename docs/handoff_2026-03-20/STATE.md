...
- PDF nutrient-name click highlights remain precision-first/table-only; continuation-page recall and cross-text-item nutrient phrase matching are still future work. Compact AI source strips cover table/paragraph/page navigation separately, map printed page hints when possible, and draw visible overlays for every matched table or paragraph region, clicked or not. Evidence overlays are scaled from PDF.js text-coordinate bounds rather than measured from rendered text-layer spans, so table and paragraph chips no longer depend on DOM span placement.

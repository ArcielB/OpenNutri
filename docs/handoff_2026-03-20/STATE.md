...
- Daily ops uses a 3-stage model cascade: `gemma_proof_extraction_v1` (`gemma-4-31b-it`, with `gemma-4-26b-a4b-it` as same-stage fallback) screens/ranks about 1500 papers/day, `gemini_flash_lite_triage_v1` (`gemini-3.1-flash-lite`) re-ranks up to about 500 top Gemma candidates/day, and `gemini_flash_db_payload_v2` (`gemini-3.5-flash`) performs the final extraction for about 20 top triage candidates/day. Scheduled ops run as recurring ticks keyed to stage-specific quota-day completion counts: Gemma defaults to UTC reset, Gemini-family stages default to `America/Los_Angeles` reset. The serialized controller tops up only enough queued/non-stale-processing Gemma work to keep about 150 active tasks, drain workers process bounded Gemma slices of 20 tasks each, then interleave Flash-Lite and final Gemini slices so useful papers can reach humans before the full Gemma target completes. If Gemma source/refill is empty while queued downstream candidates and quota remain, workers still drain Flash-Lite/final Gemini instead of exiting early. Existing Gemma backlogs above the active target should drain naturally without a new 1500-paper crawl.
...
- Scheduled ops runs storage hygiene from the serialized controller. It deletes orphan paper bucket objects and PDFs for failed/provisional/final no-data AI routes, but preserves queued Flash-Lite/final-Gemini, model-processing, and `human_review_ready` PDFs needed for extraction and labeling. The controller skips new refill when the paper bucket remains above the default 900 MB soft limit after cleanup; existing AI tasks still drain through worker jobs.
...
- `Pipeline`: cockpit-only operational view for crawler search, PDF acquisition, the three model stages, and human review. It uses the `get_pipeline_ops_snapshot` RPC to show a simple "Right Now" queue block plus an all-time-by-default funnel with a time filter. The UI labels the cascade by stable role names, with the current model spec in parentheses: `Small model (...)`, `Medium model (...)`, and `Strong model (...)`.
...
1. Controller runs storage cleanup and records estimated bytes remaining in the paper bucket.
...
4. Controller counts active Gemma work from `paper_stage_tasks.status IN ('queued', 'processing')`, excluding stale processing rows. If today's completed Gemma count is below 1500 and active Gemma work is below `--screening-active-target 150`, it crawls/uploads only `min(remaining_today, 150) - active_count` English papers using bounded 150-paper batch/chunk settings and `--refill-step-tr 0`. It also reports the soft downstream reservoir target of 500 candidates but does not evict existing tasks.
5. Controller skips new refill when the paper bucket remains above the default 900 MB soft limit after cleanup, but this does not block worker draining.
6. Workers drain at most `--screening-tick-tasks 20` Gemma tasks each, bounded by the remaining daily Gemma target and queued count. Workers never crawl/upload/refill.
7. Workers in interleaved mode drain Flash-Lite at highest-priority order for at most `--triage-tick-tasks 10`, bounded by `--triage-daily-target 500`, then final Gemini for at most `--extraction-tick-tasks 2`, bounded by `--extraction-daily-target 20`; assign any new `human_review_ready` papers immediately.
8. Fail non-quota task errors past `AI_STAGE_MAX_TASK_ATTEMPTS=2` instead of retrying forever; quota/rate-limit errors continue to requeue without consuming meaningful attempts.
9. Stop on daily targets reached, stage quota, source exhaustion/no candidates, model configuration error, dry-run, storage soft-limit refill skip, or GitHub Actions job termination. The next 5-minute tick resumes from database state.
...
- Run daily ops controller:
  - `python3 services/data-pipeline/scripts/daily_ops_orchestrator.py --json-summary --controller-only --tick-mode --stage-rpm gemma_proof_extraction_v1=20,gemini_flash_lite_triage_v1=20,gemini_flash_db_payload_v2=15 --max-wallclock-minutes 0 --screening-daily-target 1500 --screening-active-target 150 --triage-daily-target 500 --extraction-daily-target 20 --screening-refill-batch-en 150 --screening-refill-chunk-en 150 --screening-prefill-stall-limit 3 --paper-bucket-soft-limit-mb 900 --refill-step-tr 0`

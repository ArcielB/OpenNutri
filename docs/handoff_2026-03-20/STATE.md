...
- Scheduled ops no longer stores paper PDFs in Supabase Storage. The crawler still downloads and validates source PDFs locally, but `upload_to_supabase.py` persists the source `pdf_url` and registers paper metadata without paper Storage upload by default. Model workers fetch `papers.pdf_url` on demand, and the serialized controller skips paper-storage cleanup/soft-limit checks. Keep `OPENNUTRI_STORE_PDFS_IN_SUPABASE=0` unless an explicit legacy Storage run is reviewed; suggestion attachments still use their private Storage bucket.
...
6. Workers in interleaved mode drain Flash-Lite at highest-priority order for at most `--triage-tick-tasks 10`, bounded by `--triage-daily-target 500`, then final Gemini for at most `--extraction-tick-tasks 2`, bounded by `--extraction-daily-target 20`; assign any new `human_review_ready` papers immediately.

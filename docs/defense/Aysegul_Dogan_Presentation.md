...
Accurate **food-composition data** — how much protein, fat, iron or vitamin C a food actually contains — is the backbone of nutrition labels, diet apps, export compliance, clinical nutrition, and public-health policy. The problem is **how that data is built today.** Every reference database the world relies on — USDA's FoodData Central, the EU's EFSA, Turkey's TürKomp — is made the same way: **human experts transcribe values out of scientific papers by hand, one number at a time.** That manual bottleneck makes them **narrow** (only common foods — thousands that appear in the literature are in neither USDA nor EFSA), **stale** (TürKomp has barely moved since 2014), and **gated** (so costly the result is licensed for a fee).

Meanwhile **the data already exists** — published every day in thousands of papers, just locked inside unstructured PDFs full of footnotes, units, and multi-column tables. It isn't in a database because **reading it out by hand doesn't scale** — yet the obvious shortcut, "just let an AI read the papers," produces too many wrong numbers, and a wrong value on a food label is worse than none. So the real problem is: **how do you turn that ocean of published-but-unstructured data into a clean database at scale, while guaranteeing every number is correct enough to trust?**

## 2. How did we solve it (as a team)?

We built **OpenNutri**, a **"Hybrid Intelligence" pipeline** that flips the model: **the machine does the discovery and the heavy reading at scale, and the human only verifies.** The result is a database that is *automatically generated, expert-verified, citation-backed, and self-improving* — which is exactly where it beats the alternatives: unlike **USDA / EFSA / TürKomp** it reads the literature automatically and attaches **DOI-level provenance to every value**; unlike **commercial providers** (Edamam, FatSecret) it produces *new* data from primary sources instead of reselling licensed data; and unlike **a single AI model** it uses a **layered cascade** where each paper climbs through progressively more capable, more expensive stages, so only the hard, **low-confidence** cases reach the costly ones — and the papers the AI is least sure about get the most scrutiny (at the top, a human).

The full design has five layers: **(L1)** a crawler, **(L2)** a lightweight filter, **(L3)** open-source extraction models, **(L4)** a stronger commercial model for hard cases, and **(L5) expert human verification** — the final gold-standard record. It is **self-improving**: every L5 correction flows back as a training signal, so over time the AI handles more on its own and cost per paper falls — something manual curation can never do. Our implementation is one system with three surfaces over a single Supabase Postgres database: a **Python discovery & extraction pipeline** (Arciel — searches the literature, downloads PDFs, runs a working three-stage AI cascade: cheap screening on ~1,500 papers/day → medium triage → a strong PDF model on the best ~20/day = L1–L4), a **Supabase database contract** (Arciel — immutable storage, row-level security, the review workflow), and **the expert-annotator web app** (my frontend) — **Layer 5, the human-verification layer, which the rest of this document is about.**

## 3. What is my part, why is it necessary, and what did I do?

**My part is the entire expert-annotator frontend** — the **React 19 + Vite** app, deployed on **Vercel**, that every verifier and approver uses. In OpenNutri's design it is **Layer 5 — Expert Human Verification, the "Gold Standard" layer.**

**Why it is necessary: it replaces manual data entry with verification.** Every existing database is built by an expert doing manual entry — read the paper, find the table, decode units, type each number. My frontend breaks that bottleneck three ways:

1. **It turns "transcribe from scratch" into "review a draft."** Only the papers where the AI's confidence is **low** are routed to a human — the cascade spends scarce expert time exactly where the AI is unsure, not on everything. By the time a paper reaches the screen, the AI has already found it, read it, and pre-filled candidate values, so the expert **verifies and corrects** instead of transcribing — the difference between a few papers an hour and the ~8–12 verified papers/day this project needs.
2. **It makes the evidence instant instead of a manual search.** The slowest, most error-prone step in manual entry is *finding* the right value in a 15-page PDF and being sure of the row, column, and units. My UI lights up **the exact table or sentence a value came from** and makes the nutrient names in it clickable, so the expert's eyes go straight to the evidence and confirm rather than search. This is *why* my hardest work, the PDF evidence engine, had to exist.
3. **It is the only place expert-verified, gold-standard truth — and the training signal — is produced.** Manual entry yields a number and nothing else; my UI yields a number **plus** an error-categorized correction that retrains the AI, **plus** automatic DOI provenance, **plus** the immutable audit record behind the project's <0.5% error bar.

In short, **manual data entry doesn't scale and never improves; the verification UI I built does both** — and it had to do it inside the project's **free-tier** data limits (Supabase, Vercel) while still feeling instant.

**What I did, in one sentence:** I built the workspace where a labeler opens a paper, **the exact table or paragraph a value came from lights up on the PDF**, the nutrient names become clickable, a click drops the value into an editor, forgiving search maps it to the canonical food/nutrient catalog, and the validated submission is stored — all client-side, on arbitrary publisher PDFs, fast enough to do all day. Concretely, I own seven things, which the next section walks through one by one:
...
Doğru **besin bileşim verisi** — bir gıdanın gerçekte ne kadar protein, yağ, demir ya da C vitamini içerdiği — besin etiketlerinin, diyet uygulamalarının, ihracat uyumunun, klinik beslenmenin ve halk sağlığı politikasının temelidir. Asıl sorun, **bu verinin bugün nasıl kurulduğudur.** Dünyanın dayandığı her referans veritabanı — USDA FoodData Central, AB'nin EFSA'sı, Türkiye'nin TürKomp'u — aynı şekilde yapılır: **insan uzmanlar değerleri bilimsel makalelerden elle, tek tek sayı sayı kopyalar.** Bu manuel darboğaz onları **dar** (yalnızca yaygın gıdalar — literatürde görünen binlerce gıda ne USDA'da ne EFSA'dadır), **eski** (TürKomp 2014'ten beri neredeyse hiç ilerlemedi) ve **ücretli** (kurması o kadar pahalı ki sonuç ücretle lisanslanır) kılar.

Oysa **veri zaten mevcuttur** — her gün binlerce makalede yayımlanır, yalnızca dipnotlar, birimler ve çok sütunlu tablolarla dolu yapısız PDF'lerin içinde kilitlidir. Bir veritabanında olmamasının nedeni **onu elle okuyup çıkarmanın ölçeklenmemesidir** — ama bariz kısayol olan "bırak yapay zekâ makaleleri okusun" çok fazla yanlış sayı üretir ve bir besin etiketindeki yanlış bir değer, hiç değer olmamasından daha kötüdür. Yani asıl problem: **o yayımlanmış-ama-yapısız veri okyanusunu, her sayının güvenilecek kadar doğru olduğunu garanti ederken, ölçekli biçimde nasıl temiz bir veritabanına dönüştürürsün?**

## 2. Biz bunu (ekip olarak) nasıl çözdük?

Modeli tersine çeviren bir **"Hibrit Zekâ" hattı** olan **OpenNutri**'yi kurduk: **makine keşfi ve ağır okumayı ölçekte yapar, insan ise yalnızca doğrular.** Sonuç; *otomatik üretilen, uzman-doğrulamalı, atıf-destekli ve kendini geliştiren* bir veritabanıdır — ki tam da burada alternatifleri geçer: **USDA / EFSA / TürKomp'un** aksine literatürü otomatik okur ve **her bir değere DOI düzeyinde köken bilgisi** iliştirir; **ticari sağlayıcıların** (Edamam, FatSecret) aksine lisanslı veriyi yeniden satmak yerine birincil kaynaklardan *yeni* veri üretir; ve **tek bir yapay zekâ modelinin** aksine, her makalenin giderek daha yetenekli, daha pahalı aşamalardan geçtiği **katmanlı bir kademe** kullanır — böylece yalnızca zor, **düşük-güvenli** vakalar pahalı katmanlara ulaşır ve yapay zekânın en az emin olduğu makaleler en çok incelemeyi alır (en tepede bir insan).

Tam tasarımı beş katmanlıdır: **(L1)** bir tarayıcı, **(L2)** hafif bir filtre, **(L3)** açık kaynak çıkarım modelleri, **(L4)** zor vakalar için daha güçlü bir ticari model ve **(L5) uzman insan doğrulaması** — nihai altın-standart kaydı. Sistem **kendini geliştirir**: her L5 düzeltmesi bir eğitim sinyali olarak geri akar, böylece zamanla yapay zekâ daha fazlasını kendi başına halleder ve makale başına maliyet düşer — manuel derlemenin asla yapamayacağı bir şey. Uygulamamız, tek bir Supabase Postgres veritabanı üzerinde üç yüzeyli tek bir sistemdir: bir **Python keşif & çıkarım hattı** (Arciel — literatürü tarar, PDF'leri indirir, çalışan üç aşamalı bir YZ kademesi çalıştırır: günde ~1.500 makalede ucuz eleme → orta ayıklama → en iyi ~20 makalede güçlü bir PDF modeli = L1–L4), bir **Supabase veritabanı sözleşmesi** (Arciel — değiştirilemez saklama, satır düzeyi güvenlik, inceleme iş akışı) ve **uzman-etiketleyici web uygulaması** (benim frontend'im) — **Layer 5, insan-doğrulama katmanı; bu belgenin geri kalanı bununla ilgilidir.**

## 3. Benim parçam ne, neden gerekli ve neyi yaptım?

**Benim parçam, uzman-etiketleyici frontend'inin tamamı** — her doğrulayıcının ve onaylayıcının kullandığı, **Vercel**'de yayında olan **React 19 + Vite** uygulaması. OpenNutri'nin tasarımında bu, **Layer 5 — Uzman İnsan Doğrulaması, "Altın Standart" katmanıdır.**

**Neden gerekli: manuel veri girişini doğrulamayla değiştirir.** Mevcut her veritabanı bir uzmanın manuel girişiyle kurulur — makaleyi oku, tabloyu bul, birimleri çöz, her sayıyı yaz. Benim frontend'im bu darboğazı üç şekilde kırar:

1. **"Sıfırdan kopyalama"yı "taslağı gözden geçirme"ye dönüştürür.** Yalnızca yapay zekânın güveninin **düşük** olduğu makaleler bir insana yönlendirilir — kademe, kıt uzman zamanını her şeye değil, tam olarak YZ'nin emin olmadığı yere harcar. Bir makale ekrana geldiğinde YZ onu çoktan bulmuş, okumuş ve aday değerleri doldurmuştur; uzman kopyalamak yerine **doğrular ve düzeltir** — saatte birkaç makale ile bu projenin ihtiyaç duyduğu günde ~8–12 doğrulanmış makale arasındaki fark.
2. **Kanıtı manuel bir aramadan anlık hâle getirir.** Manuel girişte en yavaş ve en hataya açık adım, 15 sayfalık bir PDF'te doğru değeri *bulmak* ve doğru satır/sütun/birim olduğundan emin olmaktır. Arayüzüm **bir değerin geldiği tam tabloyu veya cümleyi** aydınlatır ve içindeki besin adlarını tıklanabilir yapar; böylece uzmanın gözü doğrudan kanıta gider, aramak yerine onaylar. En zor işimin, PDF kanıt motorunun, var olmak zorunda olmasının nedeni budur.
3. **Uzman-doğrulamalı, altın-standart gerçeğin — ve eğitim sinyalinin — üretildiği tek yerdir.** Manuel giriş bir sayı üretir, başka bir şey değil; arayüzüm bir sayı **artı** YZ'yi yeniden eğiten, hata-kategorili bir düzeltme **artı** otomatik DOI kökeni **artı** projenin % 0,5 altı hata çıtasının arkasındaki değiştirilemez denetim kaydı üretir.

Kısacası: **manuel veri girişi ne ölçeklenir ne de gelişir; benim kurduğum doğrulama arayüzü ikisini de yapar** — ve bunu, projenin **ücretsiz katman** veri limitleri (Supabase, Vercel) içinde, yine de anlık hissettirerek yapması gerekti.

**Ne yaptığım, tek cümlede:** Bir etiketleyicinin bir makaleyi açtığı ve **bir değerin geldiği tam tablonun veya paragrafın PDF üzerinde aydınlandığı**, besin adlarının tıklanabilir hâle geldiği, bir tıklamanın değeri editöre düşürdüğü, bağışlayıcı aramanın onu kanonik gıda/besin kataloğuna eşlediği ve doğrulanmış gönderinin saklandığı çalışma alanını kurdum — hepsi istemci tarafında, keyfi yayıncı PDF'leri üzerinde, bütün gün yapılabilecek kadar hızlı. Somut olarak yedi şeyin sahibiyim; bir sonraki bölüm bunları tek tek anlatıyor:

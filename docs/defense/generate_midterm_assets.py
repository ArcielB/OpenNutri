...
def generate_architecture():
    ...
    fig, ax = setup_canvas((12, 7))
    ax.text(0.5, 0.95, "OpenNutri Midterm Architecture", ha="center", va="center", fontsize=18, weight="bold", color="#0f172a")
    add_box(
        ax, 0.05, 0.62, 0.22, 0.18,
        "Scientific Sources",
        "Europe PMC\nOpenAlex\nSemantic Scholar\nDergiPark",
    ...
    add_arrow(ax, (0.27, 0.71), (0.39, 0.71), text="metadata")
    add_arrow(ax, (0.61, 0.71), (0.73, 0.71), text="accepted paper + evidence")
    add_arrow(ax, (0.84, 0.62), (0.30, 0.43), text="PDF and paper queue")
    add_arrow(ax, (0.38, 0.33), (0.62, 0.33), text="label events")
    add_arrow(ax, (0.75, 0.43), (0.75, 0.62), text="learned terms / next search")
    ax.text(
        0.5,
        0.08,
    ...
    save(fig, "figure_1_system_architecture_en.png")
...
def generate_feedback_data_model():
    ...
    fig, ax = setup_canvas((12, 7.5))
    ax.text(0.5, 0.95, "Annotation and Feedback Data Relations", ha="center", va="center", fontsize=18, weight="bold", color="#0f172a")
    add_box(ax, 0.07, 0.68, 0.18, 0.14, "papers", "Paper identity\nsource\nworkflow_language\nscore fields", fc="#dbeafe")
    add_box(ax, 0.30, 0.68, 0.18, 0.14, "annotations", "Per-user paper state\nhas_data", fc="#dcfce7")
    add_box(ax, 0.53, 0.68, 0.18, 0.14, "food_items", "Food rows\ninside the paper", fc="#dcfce7")
    add_box(ax, 0.76, 0.68, 0.18, 0.14, "annotation_nutrient_values", "Dynamic nutrient rows\nfor each food item", fc="#dcfce7")
    add_box(ax, 0.18, 0.38, 0.24, 0.15, "paper_label_events", "draft / done / skipped\nhas_data\nfood_item_count\nnutrient_value_count", fc="#fde68a")
    add_box(ax, 0.50, 0.38, 0.20, 0.15, "paper_global_labels", "definitely_no_data\nshort undo window", fc="#fde68a")
    add_box(ax, 0.75, 0.37, 0.18, 0.17, "paper_search_hits +\npaper_search_batches", "Query evidence\nbatch yield\nsource-level traceability", fc="#fce7f3")
    add_box(ax, 0.18, 0.10, 0.26, 0.16, "feedback/update_terms.py", "Derives positive / negative /\nconflict cases from events\nproduces n-grams and batch scores", fc="#e9d5ff")
    add_box(ax, 0.56, 0.10, 0.26, 0.16, "crawler_v2.py", "Uses feedback scores\nas soft evidence\nfor later candidate selection", fc="#e9d5ff")
    add_arrow(ax, (0.25, 0.75), (0.30, 0.75))
    add_arrow(ax, (0.48, 0.75), (0.53, 0.75))
    add_arrow(ax, (0.71, 0.75), (0.76, 0.75))
    add_arrow(ax, (0.30, 0.68), (0.30, 0.53))
    ...
    add_arrow(ax, (0.60, 0.38), (0.37, 0.26))
    add_arrow(ax, (0.84, 0.37), (0.69, 0.26))
    add_arrow(ax, (0.44, 0.18), (0.56, 0.18), text="learned terms,\nsource priors,\nbatch scores")
...
def generate_crawler_funnel():
    ...
    labels = [
        "TR raw hits",
        "Passed search gate",
        "Passed metadata filter",
        "PDF fetch failed",
        "PDF validation failed",
        "Accepted",
    ]
    values = [142, 137, 78, 32, 31, 3]
    subtitle = "Fallback values used because the example live-run manifest was not found."
    if FUNNEL_SOURCE.exists():
        payload = json.loads(FUNNEL_SOURCE.read_text(encoding="utf-8"))
        summary = payload.get("summary") if isinstance(payload.get("summary"), dict) else {}
        tr_row = summary.get("languages", {}).get("tr", {})
        if isinstance(tr_row, dict):
            ...
            subtitle = "Source: services/data-pipeline/data/crawl_tr_live_2026-03-30b/raw_pdfs/_harvest_metadata.json"
    colors = ["#60a5fa", "#38bdf8", "#34d399", "#f59e0b", "#fb7185", "#22c55e"]
    y_positions = list(range(len(labels)))
    ax.barh(y_positions, values, color=colors, edgecolor="#1f2937")
    ax.set_yticks(y_positions, labels)
    ax.invert_yaxis()
    ax.set_xlabel("Record / stage count")
    ax.set_title("Example Crawler Stage Summary (TR live run dated 2026-03-30)")
    for idx, value in enumerate(values):
        ax.text(value + max(values) * 0.015, idx, str(value), va="center", fontsize=9, color="#0f172a")
    fig.text(0.02, 0.02, subtitle, fontsize=8.5, color="#475569")
    fig.tight_layout(rect=(0, 0.05, 1, 1))

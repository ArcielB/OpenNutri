...
- Cockpit Pipeline must use `get_pipeline_ops_snapshot` for crawler-to-human operational visibility. It should show aggregate accepted/rejected/pass-forward counts, current stage queues, recent task errors, and per-paper traces without exposing raw model reasoning.

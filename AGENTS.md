...
- Automated daily ops uses `gemma_proof_extraction_v1` with `gemma-4-31b-it` before Gemini, with `gemma-4-26b-a4b-it` configured as the same-stage fallback for retryable 31B model failures. Gemma `has_data` outputs enqueue Gemini by priority; Gemma/Gemini `no_usable_data` outputs become provisional skips. Daily ops is a resumable tick system keyed to UTC-day completion counts: each scheduled tick only tops up enough queued Gemma papers for the next immediate processing slice, drains a bounded Gemma slice, and interleaves a small Gemini slice from already-ranked Gemma-positive candidates. If Gemma source/refill is empty while Gemini still has queued candidates and remaining daily quota, the tick still drains Gemini instead of exiting early. Existing large Gemma backlogs should drain naturally instead of triggering a new 1500-paper crawl.
- Daily ops is scheduled every 5 minutes by `.github/workflows/daily-ops.yml`. Each scheduled invocation runs one serialized `refill-controller` job under the `daily-ops-refill-controller` concurrency group, then a 5-worker drain matrix. Manual dispatches accept a `workers` input for smaller controlled bursts; inactive worker entries skip checkout/setup/cache/dependency work before the final drain step. The controller is the only scheduled path that may crawl/upload/refill. It runs unbuffered `daily_ops_orchestrator.py --controller-only --tick-mode` with `--screening-daily-target 1500`, `--screening-active-target 75`, `--screening-refill-batch-en 75`, `--screening-refill-chunk-en 75`, `--paper-bucket-soft-limit-mb 900`, and `--refill-step-tr 0`. Workers run unbuffered `daily_ops_orchestrator.py --drain-only --tick-mode --interleave-extraction` with `--screening-tick-tasks 15` and `--extraction-tick-tasks 2`; workers must never crawl, upload, or refill. Overlapping worker matrices are allowed because `claim_paper_stage_tasks` atomically claims distinct tasks. Keep this controller-plus-drain-workers model unless the ops strategy is explicitly changed.

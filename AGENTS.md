...
- Daily ops is scheduled every 5 minutes by `.github/workflows/daily-ops.yml`. The workflow intentionally does not set a global concurrency group; overlapping scheduled ticks are allowed because `claim_paper_stage_tasks` atomically claims distinct tasks and one serialized runner was too slow for the 1500/day Gemma target. Each scheduled workflow invocation fans out to 20 independent workers; manual dispatches accept a `workers` input for smaller controlled bursts. Each active worker runs one unbuffered `daily_ops_orchestrator.py --tick-mode --interleave-extraction` call with `--stage-rpm gemma_proof_extraction_v1=15,gemini_flash_db_payload_v2=15`, `--max-wallclock-minutes 0`, `--screening-daily-target 1500`, `--screening-tick-tasks 15`, `--extraction-daily-target 20`, `--extraction-tick-tasks 2`, `--screening-refill-batch-en 15`, `--screening-refill-chunk-en 15`, and `--screening-prefill-stall-limit 3`. Keep this recurring tick-worker model unless the ops strategy is explicitly changed.
...
- Scheduled ops storage hygiene runs `cleanup_paper_storage.py --apply` from matrix worker 1. It deletes orphan `papers` bucket objects and PDFs for failed/provisional/final no-data AI routes, while preserving queued Gemini/model-processing and `human_review_ready` PDFs that are still needed for extraction or labeling.

...
- Automated daily ops now runs a cheap Gemma proof-extraction stage before Gemini. Gemma `has_data` outputs enqueue Gemini by priority; Gemma/Gemini `no_usable_data` outputs become provisional skips. The daily Gemini target remains 20 calls; cheap screening has a separate larger per-invocation budget.

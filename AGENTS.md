...
- Automated daily ops uses `gemma_proof_extraction_v1` with `gemma-4-26b-a4b-it` before Gemini. Gemma `has_data` outputs enqueue Gemini by priority; Gemma/Gemini `no_usable_data` outputs become provisional skips. Daily ops is Gemma-target-led: prefill at least 1500 queued Gemma papers first, drain exactly the scheduled 1500 Gemma calls at stage RPM, then drain Gemini from highest-priority queued candidates until quota/source stop.
- Daily ops is scheduled once per UTC day at 04:17. `.github/workflows/daily-ops.yml` must not gate by current wall-clock time; delayed GitHub runners should still execute. The workflow runs one long unbuffered `daily_ops_orchestrator.py` invocation with `--stage-rpm gemma_proof_extraction_v1=15,gemini_flash_db_payload_v2=15`, `--quota-cooldown-seconds 65`, `--max-wallclock-minutes 0`, `--screening-daily-target 1500`, `--screening-refill-batch-en 1500`, `--screening-refill-chunk-en 1500`, and `--screening-prefill-stall-limit 3` inside the GitHub Actions job cap. Do not reintroduce the old 5-minute outer loop, small low-watermark Gemma refill loop, or hard 20-call Gemini budget.
- Scheduled ops sets `GEMMA_STAGE_TEXT_LIMIT_CHARS=60000`, `AI_MODEL_TASK_TIMEOUT_SECONDS=900`, `AI_STAGE_MAX_TASK_ATTEMPTS=2`, and `GEMINI_REQUEST_TIMEOUT_SECONDS=900`. Gemma should receive a capped head/tail excerpt; Gemini extraction remains uncapped unless a Gemini-specific cap is set. `process_stage_queue.py` requeues stale `processing` tasks before claiming work so cancelled runners do not strand papers, and non-quota task errors past the retry ceiling fail the task instead of retrying forever. Quota/rate-limit errors still requeue without consuming meaningful attempts.

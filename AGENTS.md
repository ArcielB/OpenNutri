...
- Queue and Approval evidence badges are broad navigation hints built from normalized payload evidence metadata. Matched table labels highlight detected table regions, matched source quotes highlight the matching line/paragraph block, page-only hints scroll without coloring the page, and unmatched hints remain visible as unverified.

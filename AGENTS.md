...
- Daily ops is scheduled once per UTC day at 07:17. `.github/workflows/daily-ops.yml` must not gate by current wall-clock time; delayed GitHub runners should still execute. The workflow runs one long `daily_ops_orchestrator.py` invocation with `--stage-rpm gemma_proof_extraction_v1=15,gemini_flash_db_payload_v2=15`, `--quota-cooldown-seconds 65`, `--max-wallclock-minutes 330`, `--screening-queue-low-watermark 30`, and `--screening-refill-batch-en 75` inside the 6-hour job timeout. Do not reintroduce the old 5-minute outer loop or hard 20-call Gemini budget.
...
- Run daily AI-max ops locally: `python3 services/data-pipeline/scripts/daily_ops_orchestrator.py --stage-rpm gemma_proof_extraction_v1=15,gemini_flash_db_payload_v2=15 --quota-cooldown-seconds 65 --max-wallclock-minutes 330 --screening-queue-low-watermark 30 --screening-refill-batch-en 75`

...
- Queue and Approval source strips are broad navigation hints built from normalized payload evidence metadata. Matched evidence snaps to whole detected table blocks or whole paragraph blocks, and every matched block renders as an always-on coordinate overlay; sources in the same table or paragraph share one deduplicated overlay. Page-only hints scroll without coloring the page, and unmatched hints remain visible as unverified. PDF navigation resolves AI `page_hint` values against detected printed page numbers from PDF headers/footers before falling back to PDF page indexes.

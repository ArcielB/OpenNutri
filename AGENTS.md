...
- Queue and Approval source strips are broad navigation hints built from normalized payload evidence metadata. Matched table labels highlight the detected table region, matched paragraph quotes expand to the surrounding paragraph block, page-only hints scroll without coloring the page, and unmatched hints remain visible as unverified. PDF navigation resolves AI `page_hint` values against detected printed page numbers from PDF headers/footers before falling back to PDF page indexes.

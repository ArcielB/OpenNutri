...
- Automated daily ops currently routes uploads directly to `gemini_flash_db_payload_v2`; the `gemma_proof_extraction_v1` cheap screening stage is retained but inactive until a compatible model is validated. Gemini `has_data` outputs enter human review, and Gemini `no_usable_data` outputs become provisional skips. The daily Gemini target remains 20 calls.
- Daily ops is scheduled once per America/Los_Angeles day at 00:17 after Gemini RPD reset. `.github/workflows/daily-ops.yml` uses separate PDT/PST UTC cron entries and gates by Pacific UTC offset, not current wall-clock minute, so delayed GitHub runners still execute. The GitHub Actions controller loops every 5 minutes until `daily_ops_orchestrator.py` returns `terminal=true`; terminal stop reasons include `daily_ai_call_budget_exhausted`, `ai_first_task_quota_limited`, `ai_stage_configuration_error`, `no_progress`, and `dry_run`. Non-terminal reasons like `ai_run_budget_exhausted`, `ai_quota_limited_after_progress`, and `max_cycles` should continue after the 5-minute sleep.
